module.exports = function(){
    ...
    function getMenuItems(res, mysql, context, complete){
        mysql.pool.query("SELECT item_id, name, cost, food_type FROM menu_items", function(error, results, fields){
            if(error){
                res.write(JSON.stringify(error));
                res.end();
            }
            context.menu_item_plural = results;
            complete();
        });
    }
    ...
    router.get('/', function(req, res){
        ...
        getMenuItems(res, mysql, context, complete);
        function complete(){
            callbackCount++;
            if(callbackCount >= 1){
                res.render('menu_items', context);
            }
    ...
    router.post('/', function(req, res){
        var mysql = req.app.get('mysql');
        var sql = "INSERT INTO menu_items (name, cost, food_type) VALUES (?,?,?)";
        var inserts = [req.body.name, req.body.cost, req.body.food_type];
